...
const clean = (p) => ({ ...p, id: Number(p.id), name: String(p.name || "Product"), category: String(p.category || "Other"), unit: String(p.unit || "Each"), price: Number(p.price || 0), stock: Math.max(0, Number(p.stock || 0)), image: String(p.image || ""), active: p.active !== false });
...
export default function CustomerD1() {
  ...
  const image = p => p.image ? <img src={p.image} alt={p.name} loading="lazy" /> : <span>{p.name.charAt(0)}</span>;
