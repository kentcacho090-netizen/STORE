...
export default function StoreApp() {
  ...
  const image = (product) => product.image ? <img src={product.image} alt={product.name} /> : <span>{product.name.charAt(0)}</span>;
  ...
  return <div className="app">
    <header className="topbar">
      ...
      <nav className="top-actions"><button className={view === "shop" ? "nav-btn active" : "nav-btn"} onClick={() => setView("shop")}>Catalog</button><button className={view === "admin" ? "nav-btn active" : "nav-btn"} onClick={() => setView("admin")}>Manage Store</button><button className="cart-btn" onClick={() => setView("shop")}>Cart <b>{itemCount}</b></button></nav>
    </header>
    ...
    {view === "admin" ? <main className="admin-page">
      ...
      <section className="categories-panel"><div><p className="eyebrow">CATEGORIES</p><h2>Organize your catalog</h2></div><div className="category-manager">{categories.map((item) => <div className="category-chip" key={item}><span>{item}</span><button onClick={() => deleteCategory(item)}>×</button></div>)}<button className="add-category" onClick={addCategory}>+ New category</button></div></section>
    </main> : <main>
