...
export default function App() {
  ...
  const cartItems = [...new Map(products.filter((p) => cart[p.id]).map((p) => [p.id, p])).values()];
  ...
  const add = (product) => {
    ...
    setToast({ id: product.id, name: displayName(product) });
    window.setTimeout(() => setAddingId(null), 350);
  };
  const undoAdd = () => {
    if (!toast) return;
