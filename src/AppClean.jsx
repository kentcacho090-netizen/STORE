...
export default function AppClean() {
      ...
      .then((data) => {
        ...
        setProducts((current) => {
          ...
          remote.forEach((p) => {
            ...
            const index = merged.findIndex((x) => `${x.id}__${x.unit}` === key);
            if (index >= 0 && !currentKeys.has(key)) merged[index] = p;
            else if (!currentKeys.has(key)) merged.push(p);
          });
  ...
  const addToOrder = (product) => {
    ...
    saveCartChange(product.id, current + 1);
    setToast({ id: product.id, name: product.name, added: 1 });
  ...
  const undoToast = () => {
    ...
    saveCartChange(toast.id, Math.max(0, current - toast.added));
    setToast(null);
  };
    ...
    <div className="app">
      ...
      {view === "settings" ? (
        ...
        <main>
          ...
          <section className="content-grid products-anchor"><div><div className="section-heading"><div><p className="eyebrow">WHOLESALE CATALOG</p><h2>Shop everyday essentials</h2></div><span>{filtered.length} products</span></div><div className="product-grid">{filtered.map((product) => <article className="product-card" key={`${product.id}-${product.unit}`}><div className="product-image">{productImage(product)}<small>{product.unit}</small></div><div className="product-info"><small className="product-category">{product.category}</small><h3>{product.name}</h3><div className="price-row"><div><span>Wholesale price</span><strong>{peso(product.price)}</strong></div><span className={product.stock < 30 ? "stock low" : "stock"}>{product.stock} available</span></div><button className="add-btn" disabled={!product.stock || addingId === product.id} onClick={() => addToOrder(product)}><span>+</span>{addingId === product.id ? " Added" : product.stock ? " Add to order" : " Out of stock"}</button></div></article>)}</div></div>
      ...
      {toast && <div className="add-toast" role="status" aria-live="polite"><div className="add-toast-icon">✓</div><div className="add-toast-copy"><strong>{toast.name}</strong><span>Added to your order</span></div><button type="button" className="add-toast-undo" onClick={undoToast}>Undo</button><button type="button" className="add-toast-close" onClick={() => setToast(null)} aria-label="Dismiss">×</button></div>}
    </div>
  );
}
